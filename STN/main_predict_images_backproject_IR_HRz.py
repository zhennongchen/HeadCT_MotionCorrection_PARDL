# PAR_corrected + iterative recon (apply on z = 0.625mm)
...
for i in range(0,x_par_image_predict.shape[0]):
    ...
    spline_gt_tz = transform.interp_func(times, gt_parameters[2,:][0] / 0.625)
    ...
    # load parameters
    pred = np.load(os.path.join(save_sub,'parameters/slice_35_to_50/pred_final.npy'),allow_pickle = True)
    ...
    spline_pred_tz = transform.interp_func(times, np.concatenate([np.asarray([0]),ff.round_diff(pred[2,:] , gt_parameters[2,:][0][1:] / 2.5, 0.5) * 2.5 / 0.625],axis = -1))
    ...
    raw_img,spacing,img_affine = basic.basic_image_processing(os.path.join(cg.data_dir,'raw_data/nii-images/thin_slice',patient_id, patient_subid, 'img-nii-0.625/img.nii.gz'))
    img = np.copy(raw_img)[np.newaxis, ...]
    ...
    # FP:
    if os.path.isfile(os.path.join(save_sub,'pred_PAR_corrected_HR_z_slice35.nii.gz')) == 0:
    ...
    # # ###### step 2: make PAR
    if os.path.isfile(os.path.join(save_sub,'pred_PAR_corrected_HR_z_slice35.nii.gz')) == 1:
        par_image_corrected = nb.load(os.path.join(save_sub,'pred_PAR_corrected_HR_z_slice35.nii.gz')).get_fdata()
    else:
        ...
        par_image_corrected = dp.cutoff_intensity(np.mean(par_c,axis=0),-1000)
        print('par correcteed image shape: ',par_image_corrected.shape)
        nb.save(nb.Nifti1Image(par_image_corrected, img_affine), os.path.join(save_sub,'pred_PAR_corrected_HR_z_slice35.nii.gz'))    

    # quantitaive for PAR_corrected
    par_image_average = np.zeros([par_image_corrected.shape[0], par_image_corrected.shape[1], 50])
    for zz in range(0,50):
        par_image_average[:,:,zz] = np.mean(par_image_corrected[:,:,  (40 + zz * 4) : (40 + (zz+1) * 4) ], -1)

    gt_image = nb.load(os.path.join(cg.data_dir,'raw_data/nii-images/thin_slice',patient_id, patient_subid, 'img-nii-0.625/img.nii.gz')).get_fdata()
    gt_image_average = np.zeros([gt_image.shape[0], gt_image.shape[1], 50])
    for zz in range(0,50):
        gt_image_average[:,:,zz] = np.mean(gt_image[:,:,  (40 + zz * 4) : (40 + (zz+1) * 4) ], -1)

    mae_par,_,rmse_par,_, ssim_par = ff.compare(par_image_average[:,:,35:45], gt_image_average[:,:,35:45],cutoff_low = -10, extreme = 1000)
